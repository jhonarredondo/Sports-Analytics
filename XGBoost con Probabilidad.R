...
write.xlsx(cbind(predicciones,tabla2), "XGB GPE probabilidad.xlsx", append = TRUE, sheetName = "XGBoost GPE")
...
mbo.ctrl = setMBOControlInfill(mbo.ctrl, crit = crit.ei)
# We will allow for exactly 25 evaluations of the objective function:
mbo.ctrl = setMBOControlTermination(mbo.ctrl, max.evals = 5L)
...
predicciones=predict(final.model, as.matrix(evaluar[,-1]))
tabla2=matrix(predicciones, ncol=7, byrow = TRUE)
predicciones=max.col(tabla2)-4
library(xlsx)
predicciones<-as.vector(predicciones)
setwd("C:/Users/Administrador/Desktop")
write.xlsx(cbind(predicciones,tabla2), "XGB DIF probabilidad.xlsx", append = TRUE, sheetName = "XGBoost DIF")
...
lrn$par.vals = list(
  nrounds             = 199,
  objective           = "multi:softmax", #interesante: se optimiza con softmax y finalmente se entrena esos parametros en softprob y da excelente
  verbose=0,
...
par = makeParamSet(
  ...
  makeNumericParam("colsample_bytree",       lower = 0.2,  upper = 0.9)
)
...
evaluar$Goles <-as.integer(as.factor(evaluar$Goles))-1
evaluar$PaisLocal=as.numeric(as.factor(evaluar$PaisLocal))
evaluar$EstiloLocal=as.numeric(as.factor(evaluar$EstiloLocal))
evaluar$PaisVisitante=as.numeric(as.factor(evaluar$PaisVisitante))
evaluar$EstiloVisitante=as.numeric(as.factor(evaluar$EstiloVisitante))
...
predicciones<-as.vector(predicciones)
setwd("C:/Users/Administrador/Desktop")
write.xlsx(cbind(predicciones,tabla2), "XGB Goles probabilidad.xlsx", append = TRUE, sheetName = "XGBoost Goles")
